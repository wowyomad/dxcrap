...
WCHAR* SHADER_PATH_urok2 = L"urok2.fx";
...
HRESULT CompileShaderFromFile( WCHAR* szFileName, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut )
{
    ...
    hr = D3DCompileFromFile(
        ...
        szEntryPoint,    // Name of the shader entry point function
        szShaderModel,   // Shader model to compile for
        dwShaderFlags,   // Shader compile options
        0,               // Effect compile options
        ppBlobOut,    // Double pointer to the compiled shader blob
        &pErrorBlob      // Double pointer to the error blob
    );
    if( FAILED(hr) )
    {
        if( pErrorBlob != NULL )
...
HRESULT InitGeometry()
{
    ...
    hr = CompileShaderFromFile(SHADER_PATH_urok2, "VS", "vs_4_0", &pVSBlob );
    if (FAILED(hr))
    {
    ...
    hr = CompileShaderFromFile(SHADER_PATH_urok2, "PS", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL, L"Невозможно скомпилировать файл FX. Пожалуйста, запустите данную программу из папки, содержащей файл FX.", L"Ошибка", MB_OK );
        return hr;
